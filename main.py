...
def gameplay_screen():
    ...
    if game_state.round_num == game_state.max_rounds+1: # checks game if game is over
        ...
        style_one = "black on white"
        style_two = "white" # colour to show feedback for controls

        option1 = Text(f"{PlayerAction.SYNERGIZE_EXTRACTION.name}", style=style_one)
        option2 = Text(f"{PlayerAction.TARGET_RICH_VEIN.name}", style=style_two)

        option1_render = Align.center(option1)
        option2_render = Align.center(option2)
        ...
        def on_press(key): # keyboard controls for choosing an option (cooperate or defect)
            if key == keyboard.Key.up:
                option1.style = style_one
                option2.style = style_two
                game_state.player_choice = PlayerAction.SYNERGIZE_EXTRACTION
            elif key == keyboard.Key.down:
                option1.style = style_two
                option2.style = style_one
                game_state.player_choice = PlayerAction.TARGET_RICH_VEIN
            elif key == keyboard.Key.enter:
                input_received_event.set()
                return False

            live.update(Group(Align.center(option1), empty_line, Align.center(option2)))

        with Live(option_group, refresh_per_second=8) as live: # for changing text instead of clearing and re-printing
